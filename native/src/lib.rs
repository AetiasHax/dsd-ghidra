...
use crate::{
    ...
    typesync::{get_type_sync_yaml, TypeSyncOptions},
};
...
/// # Safety
///
/// * `options` must point to a valid [`TypeSyncOptions`].
/// * `data` must point to an uninitialized [`UnsafeString`].
/// * `error` must point to a null char pointer or one generated by dsd-ghidra. It can also be null.
#[no_mangle]
pub unsafe extern "C" fn get_type_sync_data(
    options: *mut TypeSyncOptions,
    data: *mut UnsafeString,
    error: *mut *mut c_char,
) -> bool {
    let Some(options) = options.as_ref().cloned() else {
        if !error.is_null() {
            alloc_error("Invalid options pointer", error);
        }
        return false;
    };
    let Some(options) = unwrap_or_log(options.try_as_safe(), error) else {
        return false;
    };
    let Some(safe_data) = unwrap_or_log(get_type_sync_yaml(options), error) else {
        return false;
    };
    let Some(unsafe_data) = unwrap_or_log(safe_data.try_into_unsafe(), error) else {
        return false;
...
/// # Safety
///
/// * `data` must be generated by [`get_type_sync_data`].
/// * This function must be called only once for each [`UnsafeString`] created by [`get_type_sync_data`].
/// * `error` must point to a null char pointer or one generated by dsd-ghidra. It can also be null.
#[no_mangle]
pub unsafe extern "C" fn free_type_sync_data(data: *mut UnsafeString, error: *mut *mut c_char) -> bool {
    let Some(data) = data.as_ref().cloned() else {
        if !error.is_null() {
            alloc_error("Invalid data pointer", error);
        }
...
///
/// * `error` must point to a null char pointer or one generated by dsd-ghidra. It can also be null.
unsafe fn alloc_error<T: Debug + ?Sized>(display: &T, error: *mut *mut c_char) {
    if (*error).is_null() {
        free_error(error);
    }

    let error_bytes = format!("{display:?}").into_bytes();
